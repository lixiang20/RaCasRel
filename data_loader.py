...
class data_generator:
    def __init__(self, data, tokenizer, rel2id, id2rel_mine, num_rels, maxlen, batch_size=32):
        self.data = data
        ...
        for i in range(len(id2rel_mine)):
            ...
            tokens = ' '.join(tokenizer.tokenize(rel_text)[1:-1:2])

            token_ids, segment_ids = tokenizer.encode(first=tokens)

            rel_tokens.append(token_ids)
            segment_tokens.append(segment_ids)

        rel_tokens = seq_padding(rel_tokens)
        segment_tokens = seq_padding(segment_tokens)

        self.rel_tokens=np.repeat(np.expand_dims(rel_tokens,axis=0),batch_size,axis=0)
        self.segment_tokens=np.repeat(np.expand_dims(segment_tokens,axis=0),batch_size,axis=0)
            ...
            for idx in idxs:
                ...
                if s2ro_map:
                    ...
                    if len(tokens_batch) == self.batch_size or idx == idxs[-1]:
                        ...
                        yield [self.rel_tokens,self.segment_tokens,
                               tokens_batch, segments_batch, sub_heads_batch, sub_tails_batch, sub_head_batch, sub_tail_batch, obj_heads_batch, obj_tails_batch], None
                        tokens_batch, segments_batch, sub_heads_batch, sub_tails_batch, sub_head_batch, sub_tail_batch, obj_heads_batch, obj_tails_batch, = [], [], [], [], [], [], [], []
